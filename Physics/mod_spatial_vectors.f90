    ...
    type(vector) :: x
    integer, intent(in) :: n
    real(real64), optional, intent(in) :: e
        x%size = n
        allocate(x%data(n))        
        if( present(e) ) then
            x%data(1:n) = e
    ...
    type(matrix), intent(in) :: a
    type(lu_matrix) :: lu
    real(real64)  :: amax, dum, summ
    real(real64), allocatable  :: vv(:)
    integer :: i, j, k, imax, n , m
        ...
        do j=1,n
            do i=1,j-1
                ...
                summ = -lu%data(i,j)
                do k=1,i-1
                    summ = ieee_fma(lu%data(i,k),lu%data(k,j),summ)
                end do ! k loop
                lu%data(i,j) = -summ
            end do ! i loop
            amax = 0._wp
    ...
    do i=1,n
        ...
        if(ii /= 0) then
            do j=ii,i-1
                summ = ieee_fma( lu%data(i,j), x%data(j), summ)
            end do ! j loop
        else if(summ /= 0.d0) then
            ii = i
        end if
    ...
    do k=1,m
        do i=1,n
            ...
            if(ii /= 0) then
                do j=ii,i-1
                    summ = ieee_fma( lu%data(i,j), x%data(j,k), summ)
                end do ! j loop
            else if(summ /= 0.d0) then
                ii = i
            end if
    ...
    type(vector3), intent(in) ::a, b
    type(vector3) :: c     
    real(real64) :: cx, cy, cz, ax, ay, az, bx, by, bz
        ...
        cx = ieee_fma( az, by, 0.0D0 )
        cx = ieee_fma( ay, bz, -cx )
        
        cy = ieee_fma( ax, bz, 0.0D0 )
        cy = ieee_fma( az, bx, -cy )
        
        cz = ieee_fma( ay, bx, 0.0D0 )
        cz = ieee_fma( ax, by, -cy )
    ...
    real(real64), parameter :: O = 0
        ...
        ay = a%data(2)
        az = a%data(3)
        c%data = reshape([O,az,-ay, -az,O,ax, ay,-ax,O], [3, 3])    
    end function
