    ...
    class(rigid_body),intent(in) :: rb
    real(real64), intent(in) :: state(n_state)
    logical, intent(in), optional :: inv
    real(real64) :: I(3,3), R(3,3), q(4)
        q = state(4:7)
        R = rot(q)
        if(present(inv)) then
    ...
    real(real64) :: state(n_state)
    class(rigid_body),intent(in) :: rb
    real(real64), intent(in) :: r_A(3), q(4), v_A(3), omega(3)
    real(real64) :: R(3,3), I_C(3,3), p(3), L_A(3), c(3)
    ...
    class(rigid_body),intent(in) :: rb
    real(real64), intent(in) :: state(n_state)
    real(real64), intent(out) :: c(3), v_A(3), omega(3)
    real(real64) :: R(3,3), I_inv(3,3), q(4), p(3), L_A(3)
    ...
    class(rigid_body),intent(in) :: rb
    real(real64), intent(in) :: state(n_state)
    real(real64), intent(in) :: force(3), torque_A(3)
    real(real64) :: rate(n_state), p(3), L_A(3), c(3)
    real(real64) :: r_A(3), q(4), v_A(3), omega(3)
    real(real64) :: rp_A(3), qp(4), pp(3), Lp_A(3)
    ...
    class(rigid_body),intent(in) :: rb
    real(real64), intent(in) :: state(n_state)
    real(real64) :: KE, c(3), v_A(3), omega(3), p(3), L_A(3)
    ...
    use mod_nasa_ode
    type(rigid_body) :: rb
    real(real64) :: r_A(3), q(4), v_A(3), omega(3), c(3), I_C(3,3)
    real(real64) :: KE, omg_max, h_min, p(3), L_A(3), F(3), tau(3)
    real(real64), dimension(n_state) :: state, rate
    integer :: i, n, flag
    real(real64) :: t,t_end, t_next, h, y(n_state), yp(n_state)
    real ( kind = 8 ) abserr
    real ( kind = 8 ) relerr
